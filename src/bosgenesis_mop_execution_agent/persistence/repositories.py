...
class JsonExecutionRepository:
    ...
    def list_jobs(self) -> list[ExecutionJob]:
        return [ExecutionJob.model_validate(payload) for payload in self._snapshot.jobs.values()]

    def save_phase(self, phase: ExecutionPhase) -> None:
        self._snapshot.phases[_compound_key(phase.job_id, phase.phase_id)] = phase.model_dump(
            mode="json"
        )
        self._flush()
    ...
    def save_step(self, step: ExecutionStep) -> None:
        self._snapshot.steps[_compound_key(step.job_id, step.step_id)] = step.model_dump(
            mode="json"
        )
        self._flush()
    ...
    def add_observation(self, observation: Observation) -> None:
        self._snapshot.observations[observation.observation_id] = observation.model_dump(
            mode="json"
        )
        self._flush()
    ...
    def save_instruction(self, instruction: ExternalInstruction) -> None:
        self._snapshot.instructions[instruction.instruction_id] = instruction.model_dump(
            mode="json"
        )
        self._flush()
    ...
    def _flush(self) -> None:
        ...
        payload = json.dumps(self._snapshot.model_dump(mode="json"), indent=2, sort_keys=True)
